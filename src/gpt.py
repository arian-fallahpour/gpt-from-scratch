...
def generate(model, idx,
             ...
             repetition_penalty=1.0, repetition_penalty_exclude=None):
    exclude = None
    if repetition_penalty_exclude:
        exclude = torch.tensor(sorted(repetition_penalty_exclude), device=idx.device)
    ...
    for _ in range(max_new_tokens):
        ...
        # multiplied instead (the CTRL formulation). Tokens in repetition_penalty_exclude
        # (e.g. newline) are left alone since they're expected to recur on purpose.
